...
import { SITE_KINDS, type Site } from '@/lib/sites'
...
export default async function SitesPage() {
  ...
  const [{ data: siteRows }, { data: venueRows }, { data: dayRows }] = await Promise.all([
    admin.from('sites').select('id, venue_id, name, kind, beta, coords, links, active').order('name'),
          ...
          {groups.map((g) => (
            <div key={g.name}>
              ...
              <div className="space-y-2">
                {g.sites.map((s) => (
                  <SiteRow key={s.id} site={s} venues={venues} dayCount={dayCount.get(s.id) ?? 0} />
                ))}
              </div>
            </div>
          ))}
