...
export default async function ExpenseReportsPage() {
  ...
  const { data: reports } = await admin
    ...
    .select('id, created_at, reason, status, submitted_at, expense_items(amount)')
    .eq('profile_id', user.id)
    .order('created_at', { ascending: false })

  const rows = (reports ?? []).map((r) => ({
    ...r,
    total: round2(((r.expense_items ?? []) as { amount: number }[]).reduce((s, i) => s + Number(i.amount), 0)),
  }))
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="space-y-2 mb-12">
          {rows.map((r) => (
            <Link
              key={r.id}
              href={`/instructor/expenses/${r.id}`}
              className="flex items-center justify-between px-4 py-3 bg-zinc-900 border border-zinc-800 rounded-lg hover:border-zinc-700 transition-colors"
            >
              <div>
